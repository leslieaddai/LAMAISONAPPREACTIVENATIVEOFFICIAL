...
export default function ImageUploadLookbook(props) {
  ...
  const user = useSelector(state => state.userData)
    ...
    const [quantity,setQuantity]=useState([{color_id:'',color:'',size_id:'',size:'',quantity:''}]);
...
{loading && (
    ...
    //  )
     :nextButton && showQuantity && !confirmButton ? (
       ...
       <TouchableOpacity onPress={()=>setQuantity([...quantity,{color_id:'',color:'',size_id:'',size:'',quantity:''}])} style={{marginRight:wp2(6),marginLeft:wp2(2)}}>
