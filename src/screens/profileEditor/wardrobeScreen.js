...
export default function WardrobeScreen(props) {
  ...
  const getWardrobe = (page_no) => {
    ...
    axios
    .get(GetVirtualWardrobe+`${props?.route?.params?.user?.userData?.id}${user?.token!==''?'/'+user?.userData?.id+'?page='+page_no:'?page='+page_no}`)
...
{loadingComp && (
    ...
    <SafeAreaView style={{flex:1}}>
      ...
      contentContainerStyle={{paddingVertical:hp2(4),justifyContent:'space-between'}}
      data={data}
      numColumns={2}
      onEndReached={()=> !loading && page?.current_page < page?.last_page && getWardrobe(String(page?.current_page+1))}
      onEndReachedThreshold={0.1}
      renderItem={({item})=>{
        return(
          <WardrobeComp data={item} state={{loadingComp,setLoadingComp}} />
        )
      }}
      />
